...
public class Example1 {
    ...
    [ String 클래스 ] : 자바에서 문자열 표현하는 방법
        ...
        - 문자 번역 : 바이트 <---> 문자
            ...
            2. 유니코드     : 전세계 공용어(한글포함) , 자바가 사용한느 코드
        ...
        3. 주요 메소드
            ...
            5) 문자열A.lnegth()          : 지정한 문자열의 문자개수 반환 메소드
            ...
            9) 문자열A.indexOf("찾을문자열")    : 지정한 문자열내 찾을문자열이 존재하면 찾은문자열 반환 없으면 -1 밙환
            10) 문자열A.contains("찾을문자열")  : 지정한 문자열내 찾을문자열이 존재하면 true 없으면 false
    ...
    */
        ...
        // 1. 배열을 이용한 문자열 표현
        char str1 = '유'; // ' ' 작은 따옴표로 문자1개는 char타입
        ...
        String str6 ="유재석";             // new String 사용하지 않고 문자열 생성할경우
        String str7 ="유재석";             // 문자열이 동일한경우 동일한객체 취급
        String str8 = new String("유재석");    // -new String 사용시 동일한 문자열 이라도 다른객체 취급
        ...
        // 5) .chaAt( 인덱스 ) : 지정한 인덱스 번호의 문자 1개 반환 메소드
        char gender = "012345-1230123".charAt(7); // 1
        ...
        String str13 = "012345-1230123".substring(0,6); // 0번인덱스부터 6번 인덱스 전 까지
        System.out.println(str13); // 012345
        String str14 = "012345-1230123".substring(7); // 7번 인덱스 부터 (마지막인덱스)까지 추출
        System.out.println(str14);
        // 활용] 차량번호 조회 : 314가2294 , 차량번호 뒷자리 : 차량번호.subString(차량번호.length()-1-3);

        // 9) .split( "구분문자" ); : 지정한 문자열내 구분문자 기준으로 잘라서 배열로 변환 메소드
        ...
        boolean findout = "자바 프로그래밍 언어".contains("프로");
        System.out.println(findout); // true
        // 활용] 검색기능 구현

        // 12) .getBytes() : 지정한 문자열을 byte[] 타입으로 변환 메소드
