...
//                [경고] 더 이상 대기 등록을 할 수 없습니다.
    ...
    Waiting[] waitings = new Waiting[100]; // waitings 변수에 Waiting 객체 100개를 저장하는 배열 자료 1개 저장
        // 변수 1개 , 배열1개, 객체 100 저장공간
 ...
 for( ; ; ){ // 출력
     ...
     Scanner scan = new Scanner(System.in);
        int choose = scan.nextInt();            // 입력객체 생성
        // 선택 판단
        if( choose == 1 ) {  //  1번 선택시 입력에 따른 출력
            System.out.println("        전화번호 : "); String phone = scan.next(); // 입력 받은 값 저장
            System.out.println("        인원수 : "); int count = scan.nextInt();
            Waiting waiting = new Waiting(); // 객체생성하여 입력받은 값 대입
            waiting.phone = phone;
            waiting.count = count;

            boolean check = false; // ture : 저장성공 , false : 저장실패
            for( int index = 0; index <= waitings.length -1 ; index++){ // 배열내 빈공간을 찾아서 빈공간에 객체 대입
                if( waitings[index] == null ){ // index번째 요소가 비어(null) 있으면
                    waitings[index] = waiting;// 해당 index번째의 생성한 객체를 대입한다.
                    check = true; // 상태변경
                    break; // 반복문 종료
        ...
        }else if( choose == 2 ){ // 2번 선택시 입력에 따른 출력
            System.out.println("============= 대기 현황 =============");
            // 배열 내 요소 가 비어있지 않으면 객체 출력하시오.
            for( int index = 0; index<= waitings.length-1; index++) {
                if( waitings[index] != null){   // index번째 요소 ( 객체가) 비어있지 않으면
                System.out.println("1. 연락처:" + waitings[index].phone + "- 인원: " + waitings[index].count + "명");// 변수명[인덱스].멤버변수명
