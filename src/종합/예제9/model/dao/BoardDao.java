...
public class BoardDao {
    ...
    // ------------------------- 1. CSV 파일 연동 함수
    public void openCSV(){
        File file = new File(path); // (1) 파일 객체 생성한다. .exists() 지정한 경로에 팡리 존재여부 반환함수
        if (file.exists()){ // (2) 만약에 파일이 존재하면 정보 불러오기
            loadCSV();

        }else{ // (2) 아니면 파일 생성 .createNewFile() 지정한 경로에 파일 생성함수
            try {file.createNewFile(); // 파일 생성 , 예외 처리 한다.
