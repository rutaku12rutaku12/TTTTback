...
public class WaitingService { // class start
    public static void main(String[] args) { // main start
        ...
        for( ; ; ){ // 1) 입력
            ...
            if( choose == 1) { // 2) 1번 선택시 출력
                System.out.println("전화번호 : ");
                String phone = scan.next();
                System.out.println("인원수 : ");
                int count = scan.nextInt();

                // 4) 메소드를 호출해서 등록 처리 후 결과를 받는다.

                boolean result = // 메소드가 실행수 결과를 result에 저장,
                        controller.addWaiting( phone , count ); // 입력받은 두 값을 인수로 전달
                if( result == true ){ // addWaiting에서 true반환이면
                    System.out.println("대기 등록 성공");
                }else{ // addWaiting 에서 false 반환이면
                    System.out.println("대기 등록 실패");
                }
            }else if( choose == 2) { // 2) 2번 선택시 출력
                System.out.println("============= 대기 현황 =============");
                Waiting[] waitings = controller.getWaitingList();
                // getWaitingList() 반환된 waitings 배열 자료를 waitings에 대입한다.
                for (int index = 0; index <= waitings.length - 1; index++) {
                    Waiting waiting = waitings[index];
                    // 하나(객체) = 여러개(객체)배열[번호]
                    if (waiting != null) {
                        System.out.println("1. 연락처:" + waiting.phone + "- 인원: " + waiting.count + "명");
                        System.out.println("------------------------------------");
                    } // if end
                } // for end
