...
public class Exapmle1 { // class start
    public static void main(String[] args) { // main start
        ...
        // 2. 강제 타입변환 (명시적)
            // doubel -> float - > long - > int = > stort/char - > byte
            // (1) 메모리 크기가 큰 것 에서 작은 것으로 변환 가능
            // (2) 데이터의 손실이 크다
            // (3) (변환할타입명)(변수명);
        double dvalue = 3.14;
        float fvalue = (float) dvalue;  // double + float 강제가능.  // 손실발생
        ...
        // 3. 연산 타입변환
            // byte + byte = int , byte + short = int , int + int = int *즉 int 이하의 연산결과는 int 이다.
            // 1. int 이하의 연산결과는 int 이다.
            // 2. 연산결과는 두개 중 형 중에 더 큰 함으로 자동 타입변환.
