    ...
    public static void main(String[] args) { // main start
            ...
            [ 예외 처리 ]
                ...
                3. 예외 종류
                    1) 일반 예외 : 컴파일(실행)될때 예외 처리 코드 여부 검사 예외
                        - 입출력 : 네트워크 , 파일 , JDBC(자바-DB연동) , 주로 자바외부 통신 : Class.forName(), Thread.sleep()
        ...
        */
        ...
        try {
            ...
            System.out.println("[예외발생] String2 클래스가 없습니다.");
        }
        ...
        // [5] 실행 예외 : NumberFormatException : 정수로 타입 불가능한 에외
        try {
            String str3 = "100";    // 문자 100 저장 , 100 숫자 o
            String str4 = "1aa";    // 문자 1aa 저장 , 1aa 숫자 x
            int int3 = Integer.parseInt(str3);    // Integer.parseInt( ); // 문자열 타입을 int 타입으로 반환하여 반환 함수
            int int4 = Integer.parseInt(str4);    // "100" -> 100 , "1aa" -> 숫자가 될수없다.
        } catch ( NumberFormatException e){
            System.out.println("[예외발생5] 정수로 반환 불가능하다.");
        }

        // [6] 실행 예외 : ArrayIndexOutOfBoundsException : 존재하지 않는 인덱스호출 예외
        try {
            int[] intArray = {1, 2, 3};
            System.out.println(intArray[0]); // 0번 인덱스 존재한다.
            System.out.println(intArray[4]); // 4번 인덱스 존재하지 않는다. 오류 발생
        }catch (ArrayIndexOutOfBoundsException e) {
            System.out.println("[예외발생6] 존재하지 않는 인덱스");
        }
        ...
        // - 주의할점 : Exception클래스는 모든 예외를 처리 하므로 다중catch 맨 아래에서 정의한다.
