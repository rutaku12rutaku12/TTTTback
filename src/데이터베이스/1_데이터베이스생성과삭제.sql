...
-- [2] SQL 명령어 작성 방법
-- 1. SQL 문법을 작성한다. 2. SQL 문법 끝에 ;(세미콜론)
show databases;
-- [3] SQL 명령어 실행 방법
...
show variables like 'datadir'; # 마지막에 세미클론 잊지맑기 
# C:\ProgramData\MySQL\MySQL Server 8.0\Data , 내 컴퓨터 db 서버 로컬 경로
...
drop database mydb0714;		# 두번 실행하면 'xxxxxx'; 'database doesn't exists' 존재하지 않는 DB명		
...
# 예제1 : 데이터베이스 기본 세팅/준비 
drop database if exists test0714; # 'test0714' 이름의 데이터베이스가 존재하면 삭제
create database test0714; 		  # 'test0714' 일므으로 데이터베이스 생성하기
show databases; 				  # DB서버내 모든 데이터베이스 목록 보기
use test0714;					  # 'test0714' 이름의 데이터베이스 (사용)활성화 하기

# 현재 파일을 저장하는 방법 : ctrl+s , 강의에서는 자바 프로젝트 폴더내 같이 저장
