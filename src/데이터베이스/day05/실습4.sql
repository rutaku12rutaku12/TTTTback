...
select * from books;
select genre from books group by genre; 		# 그룹 : ~~별, ~~끼리
# 도서가 총 몇권 == 레코드수 == 도서1개당 == 레코드1개
...
select customer from orders where customer in('김민준','이서연') group by customer;
...
select book_id,count(*) from orders group by book_id having count(order_qty)>=3;
...
select * from books;
select author, count(*) as 출판도서수 from books group by author ; # 저자별 레코드수 == 출판한도서
select author, count(*) as 출판도서수 from books group by author having 출판도서수>=2 ; 
select author, count(title) from books group by author ;
select author, count(title) from books group by author having count(title)>=2;
-- [문제 11] books 테이블의 '컴퓨터' 장르 도서 중 가장 비싼 도서의 가격을 조회하세요.
select * from books;
select genre from books where genre='컴퓨터' group by genre ; 
select genre , max(price) from books where genre='컴퓨터' ; 
-- [문제 12] books 테이블의 '소설' 장르 도서들의 총 재고 수량을 조회하세요.
select * from books;
select genre , sum(stock) from books where genre='소설';
...
select * from orders;
select count(order_qty) from orders where order_date between '2023-00-00' and '2023-03-31';
