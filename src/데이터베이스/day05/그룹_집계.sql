...
-- 오류 : bpname 속성값이 동일하더라고 bprice 다를수 있기 때문에 , 
...
select count( * ) from buy;			# max( 속성명 ) :  (레코드)개수 , null 포함
...
# [*] select 키워드 간의 작성 규칙
# select 속성명 from 테이블명 where 일반조건 group by 그룹속성명 having 그룹조건 order by 정렬속성 asc/desc limit 개수;
# [*] 처리 순서
