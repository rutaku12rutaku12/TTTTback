...
# (: 정의 시작 , ): 정의 끝 , 필드명:아무거나 속성명 , int: 속성의타입 
...
create table test3( # 테이블 정의 시작
    ...
    날짜필드1 date 	, 시간필드2 time		, 날짜시간필드1 datetime ,
...
create table waiting(
    ...
    count tinyint		  # 인원수 	 # 최대100명 	  # private byte count 
);
select *from waiting;
...
create table test4(
	# 필드명 타입 제약조건
    필드명1 tinyint not null 				, -- 지정한 속성에는 null을 저장 할수 없도록 조건 설정
    ...
    constraint primary key( 필드명4 )		-- 'pk필드명4' 속성을 PK 필드로 설정
);
select * from test4;
# [7]
create table test5(
...
create table member(						-- [1] 회원테이블
    ...
    mname varchar(10) not null ,			-- 회원닉네임 : 최대10 , 공백x
...
bCode mediumint auto_increment , -- 도서코드
bName varchar(50) not null, -- 도서이름
bAuthor varchar(50) not null, -- 저자
constraint primary key(bCode)
);

create table loan( -- 대출 테이블(loan)
	lCode mediumint auto_increment , 	-- 대출코드 
    bCode mediumint ,					-- 도서코드 
    mCode smallint ,					-- 회원코드
    loanDate datetime default now()	,	-- 대출일
    returnDate datetime now(),	        -- 반납일
	constraint primary key( lCode ),	-- 위에서 선언된 '게시물번호'속성을 *PK필드로 설정  
