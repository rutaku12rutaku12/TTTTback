...
public class UserDao {
    // (*) 싱글톤 : 주로 프로그램내 하나(싱글)의 객체(톤) 표현
        // -> 주로 MVC 패턴에서 싱글톤 사용하는 레어어/계층 : view/controller/dao
            // 왜? 기능은 하나만 있어도 재사용 가능하니까. 즉] 메소드 위주 , 요리사사람1명,서빙사람1명
        // -> DTO 에서는 싱글톤 사용하지 않는다.
    ...
    private Connection conn; // DB연동 결과를 조작하는 인터페이스 , import 주의 -> java.sql 사용
    public void connect() {
        try {
        ...
        }catch ( ClassNotFoundException e){System.out.println("[경고] mysql 라이브러리 로드 실패");} // 오류발생 1)라이브러리가 진짜 없거나, 2)경로 잘못 적을때 "com.mysql.cj.jdbc.Driver"
        catch ( SQLException e){System.out.println("[경고] DB 서버의 연동 실패 ");}
    } // func end
    ...
    public boolean userInsert2( String uname , int uage ){
        try {
            ...
            // ps.setXXX( ?번호 , 값 ) : ?에 값 대입
            // SQL 문법내 첫번째 ?에 uname 변수값을 String 타입으로 대입
            ps.setString( 1 , uname );
            // SQL 문법내 두번째 ?에 uage 변수값을 int 타입으로 대입
            ps.setInt( 2, uage );
            // 4. SQL 실행하기
    ...
    public void userSelect(){
        ...
        try {
            ...
            // ResultSet 조회결과 조작 인터페이스 import 주의 java.sql사용
                // rs.next() : 조회결과 에서  레코드/행/가로/줄 하나씩 조회/이동 함수
                // rs.getXXX( 가져올속섬명 또는 순서번호 ); : 현재 레코드의 속성 열 컬럼 의 값 반환
            // 5. SQL 결과에 따른 로직/리턴/확인
            while ( rs.next() ){ // -- 다음레코드가 존재하지 않을때 까지 반복
